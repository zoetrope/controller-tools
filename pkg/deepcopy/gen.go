...
func writeHeader(pkg *loader.Package, out io.Writer, packageName string, imports *importsList, headerText string, hasApplyConfiguration bool) {
	importPackages := strings.Join(imports.ImportSpecs(), "\n")
	if hasApplyConfiguration {
		importPackages = strings.Join([]string{importPackages, `"encoding/json"`}, "\n")
	}
...
`, packageName, importPackages, headerText)
	if err != nil {
		pkg.AddError(err)
	}

	if hasApplyConfiguration {
		_, err := fmt.Fprint(out, `func deepCopy(dst interface{}, src interface{}) error {
	if dst == nil {
		return
	}
	if src == nil {
...
func (ctx *ObjectGenCtx) generateForPackage(root *loader.Package) []byte {
	...
	if err := markers.EachType(ctx.Collector, root, func(info *markers.TypeInfo) {
		...
		if copyCtx.hasApplyConfiguration {
			hasApplyConfiguration = copyCtx.hasApplyConfiguration
		}
	...
	outContent := new(bytes.Buffer)
	writeHeader(root, outContent, root.Name, imports, ctx.HeaderText, hasApplyConfiguration)
